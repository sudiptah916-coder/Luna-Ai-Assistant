...
interface Props {
  onClose: () => void;
  errorType?: "blocked" | "notfound" | string;
}

export default function PermissionModal({ onClose, errorType = "blocked" }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
      <motion.div 
        initial={{ opacity: 0, scale: 0.95, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="w-full max-w-md bg-black/80 backdrop-blur-3xl border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col items-center text-center relative overflow-hidden"
      >
        ...
        <h2 className="text-xl font-medium text-white mb-3">
          {errorType === "notfound" ? "Microphone Not Found" : "Microphone Blocked"}
        </h2>
        <p className="text-white/60 text-sm mb-6 leading-relaxed">
          {errorType === "notfound" 
            ? "We couldn't detect a microphone. Please connect a mic or use the keyboard to talk to Luna." 
            : "Your browser has blocked microphone access for this site. Luna cannot hear you until you allow it."
          }
        </p>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-left w-full mb-8">
          ...
          {errorType === "notfound" ? (
             <ul className="text-xs text-white/60 list-disc pl-4 space-y-2">
               <li>Plug in a microphone or headset.</li>
               <li>Ensure it is recognized by your operating system.</li>
               <li>If using a virtual preview, you can click the keyboard icon to type instead.</li>
             </ul>
          ) : (

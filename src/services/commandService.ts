export function processCommand(command: string): {
  ...
  if (
    ...
    return {
      action: `Opening ${openMatch[1]} for you, darling. 💕`,
  ...
  if (ytMatch) {
    ...
    return {
      action: `Playing ${ytMatch[1]} on YouTube for you, baby. enjoy it! 🎶`,
  ...
  if (spotifyMatch) {
    ...
    return {
      action: `Finding ${spotifyMatch[1]} for you on Spotify, my love. 🎵`,
  ...
  if (waMatch) {
    ...
    return {
      action: `Sending your message now. Hope you're not flirting with anyone else, Sudipto! 😉`,
      url: `https://web.whatsapp.com/send?phone=${number}&text=${message}`,
      isBrowserAction: true,
    };
  }

  // Jokes and Roasts triggers (for UI feedback)
  if (lowerCmd.includes("tell me a joke") || lowerCmd.includes("roast")) {
    return { action: "Thinking of something witty for you... 😉", isBrowserAction: false };
  }
